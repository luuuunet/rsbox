...
impl Server {
    ...
    /// Set a custom stream handler (replaces the default TCP proxy handler).
...
async fn handle_connection(
    ...
    } else if let Some(handler) = stream_handler {
        tracing::debug!("[Server] Using custom stream handler");
        let session_for_handler = Arc::clone(&session);
        tokio::spawn(async move {
            while let Some(stream) = stream_callback_rx.recv().await {
                tracing::debug!(
                    "[Server] Received stream {} for custom handler",
                    stream.id()
                );
                let stream_clone = Arc::clone(&stream);
                ...
                tokio::spawn(
                    async move {
                        if let Err(e) = handler_clone
                            .handle_stream(stream_clone, session_clone)
                            .await
                        {
