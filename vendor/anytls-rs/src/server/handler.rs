...
pub async fn read_socks_addr(stream: Arc<Stream>) -> Result<SocksAddr> {
    let stream_id = stream.id();
    tracing::debug!(
...
/// Connect outbound TCP and send SYNACK when required by protocol version.
pub async fn connect_outbound_tcp(
    session: Arc<Session>,
    stream_id: u32,
    peer_version: u8,
