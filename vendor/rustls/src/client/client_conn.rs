...
mod connection {
    ...
    impl ClientConnection {
        ...
        /// ShadowTLS v3: sign ClientHello session id before the transcript hash.
        pub fn new_with_session_id_generator(
            config: Arc<ClientConfig>,
            name: ServerName<'static>,
            session_id_generator: Arc<dyn Fn(&[u8]) -> [u8; 32] + Send + Sync>,
        ) -> Result<Self, Error> {
            Ok(Self {
                inner: ConnectionCommon::from(ConnectionCore::for_client(
                    config.clone(),
...
impl ConnectionCore<ClientConnectionData> {
    pub(crate) fn for_client(
        ...
        proto: Protocol,
        session_id_generator: Option<Arc<dyn Fn(&[u8]) -> [u8; 32] + Send + Sync>>,
    ) -> Result<Self, Error> {
        ...
        let mut input = ClientHelloInput::new(name, &extra_exts, &mut cx, config)?;
        input.session_id_generator = session_id_generator;
