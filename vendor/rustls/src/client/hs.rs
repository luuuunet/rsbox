...
pub(super) struct ClientHelloInput {
    ...
    pub(super) session_id_generator: Option<Arc<dyn Fn(&[u8]) -> [u8; 32] + Send + Sync>>,
}
...
fn emit_client_hello_for_retry(
    ...
    if let Some(ref generator) = input.session_id_generator {
        use crate::msgs::codec::Codec;

        let mut sid_data = [0u8; 32];
        config
            .provider
            .secure_random
            .fill(&mut sid_data[..28])?;
        chp_payload.session_id = SessionId::from_fixed32(sid_data);
        let temp = HandshakeMessagePayload(HandshakePayload::ClientHello(chp_payload.clone()));
        let mut hello_bytes = Vec::new();
        temp.encode(&mut hello_bytes);
        let signed = generator(&hello_bytes);
        chp_payload.session_id = SessionId::from_fixed32(signed);
        input.session_id = chp_payload.session_id;
    }
