...
import '../platform/android_libbox_support.dart';
import 'vpn_controller.dart';
import 'vpn_mode.dart';

/// Android VPN via sing-box libbox (VpnService + TUN).
class AndroidVpnController implements VpnController {
  AndroidVpnController() {
    ...
    _stateSub = SingboxRuntime.instance.vpn.stateStream.listen(_onRuntimeState);
  }

  final _runtime = SingboxRuntime.instance;
  ...
  Future<void> connect({
    ...
    final effectiveMode = mode == VpnMode.systemProxy ? VpnMode.tun : mode;
    ...
    try {
      if (!await AndroidLibboxSupport.isAvailable()) {
        throw StateError(
          '当前设备缺少 libbox 原生库。x86 模拟器请运行 scripts/fetch_android_libbox.ps1 后重新编译，'
          '或使用 arm64 真机/模拟器。',
        );
      }
      ...
      if (!await _runtime.ensureVpnPermission()) {
        throw StateError('需要 VPN 权限，请在系统弹窗中允许');
      }
      await _runtime.ensureNotificationPermission();
  ...
  static String _userFacingError(Object e) {
    ...
    if (text.contains('PERMISSION_DENIED') || text.contains('VPN permission')) {
      return '需要 VPN 权限，请在系统弹窗中允许';
    }
    if (text.contains('SetupOptions') ||
        text.contains('libbox') ||
        text.contains('UnsatisfiedLinkError')) {
      return 'Android VPN 核心库未加载（模拟器需 x86_64 版 libbox）。'
          '请运行 scripts/fetch_android_libbox.ps1 后重新编译，或使用 arm64 真机/模拟器。';
    }
