...
void KillSingBoxProcesses() {
  const wchar_t* processes[] = {L"rsbox.exe", L"sing-box.exe"};
  for (const wchar_t* image : processes) {
    wchar_t cmd_line[128];
    swprintf_s(cmd_line, L"taskkill.exe /F /IM %s /T", image);
    STARTUPINFOW si = {};
    si.cb = sizeof(si);
