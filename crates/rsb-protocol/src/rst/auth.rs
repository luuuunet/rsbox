//! RST HTTP/3 auth headers (ALPN h3), Hy2-shaped.

use http::{HeaderMap, StatusCode};

/// Megabit/s → byte/s (sing-quic / Hy2).
pub const MBPS_TO_BPS: u64 = 125_000;

pub const AUTH_PATH: &str = "/auth";
pub const AUTH_AUTHORITY: &str = "rst";
...
pub fn parse_auth_request(headers: &HeaderMap) -> Option<AuthRequest> {
    let password = headers.get("rst-auth")?.to_str().ok()?.to_string();
...
pub fn build_auth_response(udp_enabled: bool, down_mbps: u32) -> (StatusCode, HeaderMap) {
    ...
    if down_mbps == 0 {
        headers.insert("rst-cc-rx", "0".parse().unwrap());
    } else {
        let bps = down_mbps as u64 * MBPS_TO_BPS;
        headers.insert("rst-cc-rx", bps.to_string().parse().unwrap());
    }
    let padding = random_padding(64, 512);
    headers.insert("rst-padding", padding.parse().unwrap());
...
pub fn is_auth_request(method: &http::Method, path: &str) -> bool {
