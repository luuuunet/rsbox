//! RST TCP/UDP relay after HTTP/3 auth.
...
pub async fn handle_tcp_stream(
    ...
    if dest::is_forbidden_dest(addr, ctx.allow_private) {
        let err_buf = encode_tcp_response(false, "destination forbidden", 0);
        let _ = send.write_all(&err_buf).await;
        return Ok(());
    }
    ...
    let remote = match TcpStream::connect(addr).await {
        Ok(s) => s,
        Err(err) => {
            let err_buf = encode_tcp_response(false, &format!("connect: {err}"), 0);
            let _ = send.write_all(&err_buf).await;
            return Ok(());
        }
    };
    ...
    let limiter = ctx
        .connections
        .user_limiter(&user_name, ctx.speed_bps(&limits));

    let (mut remote_read, mut remote_write) = remote.into_split();
    let inner = Arc::new(RstRelayInner {
    ...
    let c2r = copy_recv_to_tcp(&inner, &mut recv, &mut remote_write, true);
    let r2c = copy_tcp_to_send(&inner, &mut remote_read, &mut send, false);
    tokio::pin!(c2r);
    tokio::pin!(r2c);
    tokio::select! {
...
async fn copy_recv_to_tcp(
    inner: &Arc<RstRelayInner>,
    recv: &mut RecvStream,
    ...
    loop {
        ...
        record_traffic(inner, n as u64, uplink);
    }
    Ok(())
}

async fn copy_tcp_to_send(
    inner: &Arc<RstRelayInner>,
...
fn record_traffic(inner: &RstRelayInner, n: u64, uplink: bool) {
    if uplink {
